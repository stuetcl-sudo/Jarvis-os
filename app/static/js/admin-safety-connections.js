...
function createSafetySinglePicker(id, label, domains, helpText = "") {
  ...
  const items = discoveredHomeAssistantEntities.filter((item) => {
  ...
  function selectedValue() {
    return list.querySelector('input[type="radio"]:checked')?.value || "";
  }

  function render(query = "") {
    const current = selectedValue() || list.dataset.selectedValue || "";
    const needle = query.trim().toLowerCase();
    const visible = items.filter((item) => {
      const haystack = `${item.name || ""} ${item.entity_id || ""}`.toLowerCase();
      return !needle || haystack.includes(needle);
    });
    const emptyRow = element("label", "entity-checkbox-row");
    const emptyRadio = element("input");
    emptyRadio.type = "radio";
    emptyRadio.name = id;
    emptyRadio.value = "";
    emptyRadio.checked = !current;
    const emptyText = element("span", "entity-checkbox-text");
    emptyText.append(element("strong", "", "Ikke valgt"), element("small", "", "Brug automatisk/ukendt status"));
    emptyRow.append(emptyRadio, emptyText);
    const rows = visible.map((item) => {
      const row = element("label", "entity-checkbox-row");
      const radio = element("input");
      radio.type = "radio";
      radio.name = id;
      radio.value = item.entity_id;
      radio.checked = current === item.entity_id;
    ...
    list.replaceChildren(emptyRow, ...(rows.length ? rows : [element("p", "muted", "Ingen matchende entiteter.")]));
  }

  list.addEventListener("change", () => {
    list.dataset.selectedValue = selectedValue();
  });
...
applyHomeAssistantEntitySettings = function applyHomeAssistantEntitySettingsWithSafety(settings) {
  ...
  setSelectedValues("haInternetStatusEntity", [settings.internet_status_entity].filter(Boolean));
  const internetList = document.getElementById("haInternetStatusEntity");
  if (internetList) internetList.dataset.selectedValue = settings.internet_status_entity || "";
...
homeAssistantEntitySettingsPayload = function homeAssistantEntitySettingsPayloadWithSafety() {
  return {
    ...
    internet_status_entity: selectedValues("haInternetStatusEntity")[0] || "",
